...
class Chef
  ...
  # Show all the values of the Table chefs
  def self.all
  ...
  # Show the information of an specific ID
  def self.find(value)
    Chef.db.execute("SELECT * FROM chefs WHERE id = ?", value).first
  end

  # Delete all the information of an specific ID
  def self.delete(value)
    Chef.db.execute ("DELETE FROM chefs WHERE id = #{value}")
  end

  # Creating table with 7 values with a principal ID
  ...
  def self.db
    @@db ||= SQLite3::Database.new("mundito.db")
  end
